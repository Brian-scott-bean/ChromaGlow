...
**Follow-up integration (same day, after the main/staging push).** Charles's four reviewed core commits were cherry-picked (-x) onto the feature branch: B-17 (one reconciling load per external refresh, armed only by a frame that actually changed the snapshot), B-18 (the Forget cache clear runs NonCancellable so a last-bridge Forget followed by home close still clears), the Frank residual (BridgeRejected settles AMBIGUOUS for the ledger — stamp kept, wire unknown — while the UI still rolls back and reports REJECTED_BY_BRIDGE), and the D-05 residual (the boundary guard also scans `AppShellController`, `AppShellViewModel` and `MainActivity`). Four new pinning tests. `versionCode` bumped 3 → 4 because the published versionCode 3 hash no longer identifies this tree. Feature head `6d53925731bc3e5af72047a1fcb00834a2271f39`; twice-green gate: 708 passed / 0 failed / 0 skipped, lint 0 errors (28 warnings), assembleDebug OK, APK bit-identical across both passes, SHA-256 `041e1be77da832b26aba314b9ede2da33da44c52484045c0ec31dc4d80759345`. `main`/`staging` remain at `097c372` and are NOT advanced by this follow-up.
